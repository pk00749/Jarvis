...
#### 5.3.4 音频技术规范要求
**音频采样率规范**：
- **强制要求**：系统中所有音频处理必须统一使用 **24000Hz** 采样率
- 音频输入预处理时转换为24000Hz
- 语音识别模型输入采用24000Hz
- 语音合成输出采用24000Hz
- 音频播放和存储统一使用24000Hz
...
**语音合成技术规范**：
- **强制要求**：音频处理必须使用 `inference_instruct2` 方法
- 禁止使用其他推理方法（如inference_zero_shot等）
- `inference_instruct2` 是唯一指定的语音合成接口
- 该方法支持指令模式，能更好地控制粤语语音特性
- 确保与指令文本（如"请用粤语说话"）的正确处理

**技术实现要求**：
```python
# 必须使用的调用方式
cosyvoice_instance.inference_instruct2(
    text_generator,      # 文本生成器
    instruct_text,       # 指令文本："请用粤语说话"
    prompt_speech_16k,   # 提示音频（需转换为16kHz用于模型输入）
    stream=True          # 启用流式处理
)
```
